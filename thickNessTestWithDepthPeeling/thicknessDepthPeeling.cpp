...
unsigned int prePeelingFBO;//保存所有颜色的fbo
unsigned int allColorTex;//保存所有颜色的纹理数组
...
int main()
{
	...
	CWindow::setWindowName("depth_peeling_thick");
	...
	//	gBufferShader.setMat4("view", staticCameraView1);
		...
		depthPeelingShader.setMat4("projection", lightProjection);
		depthPeelingShader.setMat4("view", lightView);
		...
		glBindTexture(GL_TEXTURE_2D_ARRAY, allDepthMap);
...
void setFrameBuffers()
{
	...
	//glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, allDepthMap, 0);//FBO中不能同时拥有depthArray和colorArray
...
void setAllDepthMap()
{
	glGenTextures(1, &allDepthMap);
	glBindTexture(GL_TEXTURE_2D_ARRAY, allDepthMap);
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_DEPTH_COMPONENT, SCR_WIDTH, SCR_HEIGHT, allDepthLayer, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_COMPARE_MODE, GL_NONE);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
