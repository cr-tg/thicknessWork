...
//	outColor = vec4(LinearizeDepth(uintBitsToFloat(fragmentsLight[0].y)));
	sortSsboWithFragmentList(fragCountLight);
//	memoryBarrier();
	//uint firstIndex = imageLoad(uLightHeadPtrTex, ivec2(gl_FragCoord.xy)).r;
	//outColor = vec4(LinearizeDepth(nodeLightSet[firstIndex].depth));
...
void calThick(int fragCount)
{
	...
	//outColor = vec4(thick/255.0f*10.0f);
...
void testCalThick(int fragCount)
{
	...
	outColor = vec4(LinearizeDepth(uintBitsToFloat(nodeLightSet[preIndex].depth)));
